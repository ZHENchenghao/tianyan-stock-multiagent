...
1. **AI的投资建议不可追责** — 直接问LLM"某股票能不能买"，得到的回答自信、笼统、不可验证：错了无法追究哪一环错了，对了也无法复现。一个说不出"什么情况下我错了"的建议，等于没有建议；
2. **单一视角必然偏见** — 无论人还是单个模型，看多时会选择性忽略利空。只有让多个独立维度互相制衡、且禁止它们互相商量，结论才谈得上客观；
3. **回测普遍在说谎** — 不剥离涨停不可成交、不算真实换手成本的回测，数字再漂亮也是幻觉（本项目曾解剖一个"年化+90%"的信号，加回真实约束后为年化-21.5%）。
